...
class WaterOrder(models.Model):
    ...
    water_source_type = models.CharField("واحد مقدار آب", max_length=16, choices=[("liter", "لیتر"), ("time", "ساعت")]) # liter = tanker must transport, time = water transfer by pipes
    amount = models.IntegerField("مقدار آب", default=0)
    pump_count = models.IntegerField("پمپاژ محدد", default=0)
    pipe_length_price_type = models.CharField("نوع هزینه لوله", max_length=16, default="fix", choices=[("free", "رایگان"), ("fix", "ثابت"), ("dynamic", "متغیر")])
    ...
    def tanker_count(self):
        if self.water_source_type == "liter":
            return self.amount // self.driver.capacity + 1
        else:
            return None
    ...
    def total_payment(self):
        try:
            return accounting_models.Payment.objects.filter(order=self).aggregate(models.Sum('amount'))['amount__sum'] or 0
        except:
            return 0
